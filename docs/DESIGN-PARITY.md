...
| ------------------------------------------------------------ | ------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `scroll-snap-type: y mandatory` on `<html>` at every width   | `mandatory` from `desk` up, `proximity` below it        | `mandatory` forces the resting position onto a snap point, so a slide taller than the viewport has its tail permanently unreachable. Desktop slides are a fixed height and cannot overflow, so mandatory is safe there. A phone slide has a floor, not a fixed height, and can grow past the screen at a large accessibility text size — `proximity` still lands the deck on a slide edge but never strands text |
| Slides fixed at `calc(100vh - 79px)` with `overflow: hidden` | At least `100svh` below `desk`, growing to fit its copy | The artboard's fixed height clips the description at 390px. The first port floored it at a flat 560px, which is _shorter_ than every phone viewport bar the SE — a Pixel 7 showed 1.5 slides at once. `svh` is the right unit: `vh` is Safari's large viewport and overshoots while the toolbar shows, and `dvh` reflows mid-scroll                                                                              |
...
| Artboard                                                                                                     | Implementation                                                                                                | Why                                                                                                                                                                                                                                                                                                            |
| ------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| The whole booking screen — calendar grid, slot picker, Reserve button and confirmation                       | Calendly, embedded inline in the right column; the aside is unchanged                                         | The hand-built calendar had no connection to anyone's real calendar, so it would accept a slot already committed elsewhere, and nothing in the codebase could know otherwise. Reminders, reschedule, cancellation, timezone correctness and the meeting link all come with a scheduler. See docs/DEPLOYMENT.md |
...
| `hello@heirloomscents.com` in the footer, the About contact block, the booking aside and the structured data | Removed entirely; About's third contact column routes to the booking form instead                             | The mailbox does not exist, and the artboards flagged it themselves ("Email & phone stubs, to confirm"). A `mailto:` that bounces is worse than no link, so an e2e test now fails on any `@heirloomscents.com` address or `mailto:` appearing on any route                                                     |
| The film play badge drawn with the ▶ character (U+25B6)                                                      | A Lucide `Play` inline SVG                                                                                    | macOS and iOS give U+25B6 an emoji presentation, so the badge rendered as a colour emoji. The classical readme's instruction is "Use Lucide icons throughout" and its manifest specifies "Lucide — inline SVG on currentColor", so the artboard's glyph was the shortcut, not the system's intent              |
| The film lightbox's arrows flanking the player at every width                                                | Below `desk` the row wraps: player on its own line, both arrows centred beneath it                            | The player is capped at 80vw, so 80vw + two 46px arrows + two 28px gaps + 48px padding overflows every phone — previous sat at x=-35 and next ran 35px past the right edge on a 390px screen. Above `desk` the artboard's single row is untouched                                                              |
