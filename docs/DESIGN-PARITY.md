...
| `host@heirloomscents.com — your Heirloom host` in the confirmation's Guests row                              | "Your Heirloom host"                                                                                          | Same reason. That address survives as internal plumbing only, because the `.ics` format requires an organizer; `BOOKING_HOST_EMAIL` overrides it once email is wired                                                                                                                              |
